...
const K_VAULT  = "pm_vault";
const SENTINEL = "VAULT_OK_V1";

// FIXED: getSalt always reads existing salt first, only creates new one if truly absent
function getOrCreateSalt() {
  const stored = localStorage.getItem(K_SALT);
  if (stored) {
    try {
      return Uint8Array.from(atob(stored), c => c.charCodeAt(0));
    } catch { /* corrupted – fall through to create new */ }
  }
...
async function verifyKey(key) {
  ...
  if (!enc) return false;
  try { return (await aesDecrypt(enc, key)) === SENTINEL; }
  catch { return false; }
}

// FIXED: vault uses the SAME key that was verified at login — passed explicitly
async function loadVault(key) {
  const enc = localStorage.getItem(K_VAULT);
  if (!enc) return [];
  try { return JSON.parse(await aesDecrypt(enc, key)); }
  catch { return []; }
}
...
async function exportBackup(vault, backupPw) {
  ...
  return JSON.stringify({ v: 1, salt: btoa(String.fromCharCode(...salt)), data: enc, at: new Date().toISOString() });
}

async function importBackup(text, backupPw) {
  const b = JSON.parse(text);
  if (b.v !== 1) throw new Error("Bad format");
...
.auth-sub{color:var(--mu);font-size:14px;margin-top:6px;text-align:center}
...
.eye{position:absolute;right:12px;background:none;border:none;cursor:pointer;color:var(--mu);padding:0;display:flex;align-items:center;transition:color .2s}
...
.btn-p:hover{transform:translateY(-1px);box-shadow:0 6px 24px rgba(124,58,237,.45)}
...
.ok{background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.22);color:var(--grn);border-radius:var(--rs);padding:10px 14px;font-size:13px;margin-top:10px;text-align:center}
...
.ibtn{width:36px;height:36px;background:var(--s2);border:1px solid var(--bd);border-radius:9px;
  display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--mu);transition:all .2s}
...
.fab{width:54px;height:54px;border:none;border-radius:16px;
...
.sitem:hover{border-color:rgba(255,255,255,.12)}
...
function AuthScreen({ onAuth }) {
  ...
  async function submit() {
    setErr(""); setBusy(true);
    try {
      const salt = getOrCreateSalt();          // always reads existing salt
  ...
  return (
    <div className="auth">
      ...
      <div className="auth-sub">{isNew?"Create your master password":"Enter your master password to unlock"}</div>
      ...
      <div className="mdl">
        ...
        <div className="mbdy">
          <div className="inf" style={{marginBottom:16}}>
            ...
            <div>Your vault will be encrypted with a <b style={{color:"var(--tx)"}}>separate backup password</b>. Save the downloaded file to Google Drive, WhatsApp, or email.</div>
          </div>
          <div className="ig"><label className="lbl">Backup Password</label>
            <PwInp value={pw} onChange={setPw} placeholder="Choose backup password" strength/></div>
          ...
          {err && <div className="err">{err}</div>}
          {done && <div className="ok">✓ File downloaded! Store it somewhere safe.</div>}
      ...
      <div className="mdl">
        ...
        <div className="mbdy">
          {!preview ? <>
            <div className="inf" style={{marginBottom:16}}>
              <Ic d={I.info} size={16} color="var(--ac2)" sw={2} style={{flexShrink:0,marginTop:1}}/>
              <div>Select your <b style={{color:"var(--tx)"}}>vault-backup-*.json</b> file and enter the backup password you used when exporting.</div>
            </div>
            ...
            <div className="ok" style={{marginBottom:16}}>✓ Found {preview.length} entries in backup</div>
            <div className="fsec">
              ...
              {mode==="replace" && (
                <div className="wrn" style={{marginTop:11}}>
                  ...
                  <div>This will delete all <b>{current.length}</b> current entries and replace with the <b>{preview.length}</b> from backup.</div>
                </div>
              )}
            </div>
  ...
  return (
    <div className="spage">
      ...
      <div className="sitem" onClick={()=>setShowExp(true)}>
        <div className="sicobox" style={{background:"rgba(34,197,94,.14)"}}>
      ...
      <div className="sitem" onClick={()=>setShowImp(true)}>
        <div className="sicobox" style={{background:"rgba(99,102,241,.14)"}}>
      ...
      <div className="inf" style={{marginBottom:0}}>
        <Ic d={I.info} size={16} color="var(--ac2)" sw={2} style={{flexShrink:0,marginTop:2}}/>
        <div>
          <b style={{color:"var(--tx)"}}>Before uninstalling</b> — go to Export Backup, set a backup password, download the file, and save it to WhatsApp, email, or Google Drive.<br/><br/>
          <b style={{color:"var(--tx)"}}>After reinstalling</b> — create a new master password, then go to Import Backup, pick the file, enter your backup password, and restore.<br/><br/>
          The backup file uses its own encryption password, separate from your master password.
        </div>
      </div>

      <div className="shead">Security</div>
      <div className="sitem" onClick={onLogout}>
        <div className="sicobox" style={{background:"rgba(239,68,68,.12)"}}>
    ...
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mdl">
        ...
        <div className="mbdy">
          ...
          <div className="fsec">
            <div className="fsec-t">Images</div>
            ...
            <div className="upz" onClick={()=>fileRef.current.click()}>
              ...
              <div style={{fontSize:11,marginTop:3,opacity:.6}}>Screenshots, cards, documents</div>
            </div>
    ...
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="mdl">
        ...
        {item.images?.length>0 && <>
          <div style={{padding:"12px 18px 4px",fontSize:11,color:"var(--mu)",fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>Images</div>
          <div className="igrid">
...
export default function App() {
  const [key,     setKey]     = useState(null);   // CryptoKey — the single source of truth
  ...
  // ── Login: derive key, verify it, THEN load vault with that same key ──────
  async function handleAuth(derivedKey) {
    const items = await loadVault(derivedKey);   // uses the just-verified key
    setKey(derivedKey);
    setVault(items);
  }
  ...
  async function handleDelete(id) {
    await persist(vault.filter(v=>v.id!==id));
  ...
  return (
    ...
    <div className="app">
      ...
      {tab==="vault" && <>
        <div className="hdr">
          <div className="hdr-row">
            ...
            <button className="ibtn" onClick={handleLogout} title="Lock"><Ic d={I.logout} size={17}/></button>
          </div>
          <div className="sbox">
      ...
      {/* ── Modals ── */}
      {detail && !editing && (
        <ItemDetail item={detail} onEdit={()=>{setEditing(detail);setDetail(null);}} onDelete={()=>handleDelete(detail.id)} onClose={()=>setDetail(null)}/>
      )}
      {(adding||editing) && (
